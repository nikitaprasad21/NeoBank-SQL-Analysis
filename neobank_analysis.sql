...
-- 1. How many unique nodes are there on the Neo Bank system?
...
SELECT customer_transactions.txn_type, COUNT(*) AS transaction_count, 
      sum(customer_transactions.txn_amount) as transaction_amount
...
-- 8. For each month - how many  Neo  Bank customers make more than 1 deposit and either 1 purchase or 1 withdrawal in a single month?
...
WITH CTE AS (
         ...
         FROM customer_transactions
         GROUP BY customer_id, DATEPART(month_id,txn_date), DATENAME(month, txn_date))

SELECT month_id, month_name, count(*)
FROM CTE 
...
CTE_2 AS (
          SELECT customer_id, month_id, month_name,
              (deposit + purchase + withdrawal) as total_amt
          from CTE)

SELECT customer_id, month_id, month_name,  
        SUM(total_amt) OVER (PARTITION BY customer_id ORDER BY customer_id, month_id 
                             ROWS BETWEEN UNBOUNDED PRECEDING AND current ROW) AS balance,
        total_amt AS change_in_balance 
FROM CTE_2;
...
CTE_3 AS (
           SELECT customer_id, 
              SUM(total_amt) OVER (PARTITION BY customer_id ORDER BY customer_id, month_id, month_name
              ROWS BETWEEN UNBOUNDED PRECEDING AND current ROW) AS balance,total AS change_in_balance 
           FROM CTE_2),
...
SELECT CAST(COUNT (customer_id) AS FLOAT) * 100 / (SELECT COUNT (DISTINCT customer_id) from customer_transactions) 
